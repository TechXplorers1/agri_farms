    ...
    val configureNdk = Action<Project> {
        ...
        if (androidExtension != null) {
            try {
                ...
                method.invoke(androidExtension, "27.0.12077973")
                        println("Successfully set ndkVersion to 27.0.12077973 on project ${project.name}")
            } catch (e: Exception) {
                try {
                    val setter = androidExtension.javaClass.getMethods().firstOrNull { it.name == "setNdkVersion" }
                    setter?.invoke(androidExtension, "27.0.12077973")
                        println("Successfully set ndkVersion to 27.0.12077973 on project ${project.name} via matching name")
                } catch (e2: Exception) {
