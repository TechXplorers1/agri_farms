...
class BookServiceDetailScreen extends StatefulWidget {
  ...
  @override
  State<BookServiceDetailScreen> createState() =>
      _BookServiceDetailScreenState();
}
...
class _BookServiceDetailScreenState extends State<BookServiceDetailScreen> {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Icon(
          icon,
          size: 20,
          color: onPressed == null ? Colors.grey[400] : const Color(0xFF00AA55),
        ),
      ),
    );
  }
  ...
  bool _isSlotBlockedForDate(DateTime date, int hour) {
    ...
    for (var booking in _existingBookings) {
      if (booking.scheduledStartTime != null &&
        ...
        try {
          notes = jsonDecode(booking.notes ?? '{}');
        } catch (_) {}
        ...
        if (notes.containsKey('slots_list')) {
          try {
            bookedHours =
                (notes['slots_list'] as List<dynamic>)
  ...
  void _onSlotTap(int hour) {
    ...
    bool isDroneSpraying =
        serviceLower.contains('drone') || serviceLower.contains('spray');
  ...
  String get _fullStructuredAddress {
    ...
    if (_houseNoController.text.trim().isNotEmpty)
      parts.add(_houseNoController.text.trim());
    if (_streetController.text.trim().isNotEmpty)
      parts.add(_streetController.text.trim());
    if (_villageController.text.trim().isNotEmpty)
      parts.add(_villageController.text.trim());
    if (_mandalController.text.trim().isNotEmpty)
      parts.add(_mandalController.text.trim());
  ...
  Future<void> _loadAddress() async {
    ...
    setState(() {
      ...
      if (_fullStructuredAddress.isEmpty) {
        String savedAddr = prefs.getString('user_address') ?? '';
        if (savedAddr.isNotEmpty) {
          _addressController.text = savedAddr;
          final parts = savedAddr.split(',').map((e) => e.trim()).toList();
          if (parts.isNotEmpty) _villageController.text = parts[0];
  ...
  double get _totalPrice {
    ...
    try {
      final match = RegExp(
        r'([0-9]+(?:\.[0-9]+)?)',
      ).firstMatch(widget.priceInfo);
      if (match != null) {
        unitPrice = double.tryParse(match.group(1)!) ?? 0.0;
      }
    } catch (_) {}

    if (unitPrice == 0.0) return 0.0;

    double opPrice = _includeOperator ? (widget.operatorPrice ?? 0.0) : 0.0;
    double effectiveRate = unitPrice + opPrice;
    ...
    bool isAcreBilled =
        serviceLower.contains('harvest') ||
        serviceLower.contains('drone') ||
        serviceLower.contains('spray') ||
        serviceLower.contains('irrigation') ||
    ...
    if (isAcreBilled) {
      ...
      double effectiveQty = qty > 0 ? qty : 1.0;
      return effectiveRate * effectiveQty;
    } else {
      int count = _selectedSlots.isNotEmpty ? _selectedSlots.length : 1;
      return effectiveRate * count;
    }
  }
  ...
  void _confirmBooking() async {
    final serviceLower = widget.serviceName.toLowerCase();
    bool isElectrician = serviceLower.contains('electric');
    bool isNoSlotRequired = false;
    bool isVetCare =
        serviceLower.contains('vet') ||
        serviceLower.contains('animal') ||
        serviceLower.contains('paw') ||
        serviceLower.contains('pet');
    bool isMechanic = serviceLower.contains('mechanic');
    bool isSoilTesting =
        serviceLower.contains('soil') || serviceLower.contains('test');
    bool isPloughing =
        serviceLower.contains('plough') || serviceLower.contains('plow');
    bool isHarvesting = serviceLower.contains('harvest');
      ...
      isQtyValid =
          (_selectedPurpose != null &&
              (_selectedPurpose != 'Others' ||
                  _customPurposeController.text.isNotEmpty)) &&
          (_selectedAssetType != null &&
              (_selectedAssetType != 'Others' ||
                  _customAssetController.text.isNotEmpty));
    } else if (isVetCare) {
      isQtyValid =
          _selectedAnimalType != null && _quantityController.text.isNotEmpty;
    } else if (isMechanic) {
      isQtyValid =
          _selectedMachineryType != null && _quantityController.text.isNotEmpty;
    } else {
    ...
    if (_selectedDate != null &&
      ...
      String timeStr = isNoSlotRequired
          ? (isElectrician
              ? 'To be estimated after inspection'
              : 'Full Day / As scheduled')
          : _selectedSlots
              .map((hour) => _formatTimeRange(hour))
              .join(', ');
      ...
      final Map<String, dynamic> notesMap = {
        ...
        'Service':
            (widget.equipmentName != null && widget.equipmentName!.isNotEmpty)
                ? '${widget.serviceName} - ${widget.equipmentName}'
                : widget.serviceName,
        'Location':
            _fullStructuredAddress.isNotEmpty
        ...
        'Operator Included': _includeOperator ? 'Yes' : 'No',
        'Notes': _notesController.text,
      };

      if (isElectrician) {
        notesMap['Purpose of Visit'] =
            _selectedPurpose == 'Others'
                ? _customPurposeController.text
                : _selectedPurpose;
        notesMap['Asset to Repair'] =
            _selectedAssetType == 'Others'
                ? _customAssetController.text
                : _selectedAssetType;
      } else if (isVetCare) {
      ...
      if (isPloughing) {
        ...
        if (_selectedCropType != null) {
          notesMap['Crop Type'] = (_selectedCropType == 'Others' && _customCropTypeController.text.trim().isNotEmpty)
              ? _customCropTypeController.text.trim()
              : _selectedCropType;
        }
      }

      int startHour = _selectedSlots.isNotEmpty ? _selectedSlots.first : 9;
      int endHour = _selectedSlots.isNotEmpty ? _selectedSlots.last + 1 : 18;
      ...
      setState(() {
        ...
        if (isElectrician) {
          if (_selectedPurpose == null)
            _fieldErrors['purpose'] = 'Please select purpose of visit';
          else if (_selectedPurpose == 'Others' &&
              _customPurposeController.text.isEmpty)
            _fieldErrors['purpose_custom'] = 'Please specify purpose';

          if (_selectedAssetType == null)
            _fieldErrors['asset'] = 'Please select asset type';
          else if (_selectedAssetType == 'Others' &&
              _customAssetController.text.isEmpty)
            _fieldErrors['asset_custom'] = 'Please specify asset name';
        } else if (isVetCare) {
          if (_selectedAnimalType == null)
            _fieldErrors['purpose'] = 'Please select animal type';
          if (_quantityController.text.isEmpty)
            _fieldErrors['qty'] = 'Please enter number of animals';
        } else if (isMechanic) {
          if (_selectedMachineryType == null)
            _fieldErrors['purpose'] = 'Please select machinery type';
          if (_quantityController.text.isEmpty)
            _fieldErrors['qty'] = 'Please enter model or brand name';
        } else if (isSoilTesting) {
          if (_quantityController.text.isEmpty)
            _fieldErrors['qty'] = 'Please enter number of soil samples';
        } else if (isPloughing) {
          if (_selectedPloughEquipmentType == null)
            _fieldErrors['purpose'] = 'Please select equipment type';
          else if (_selectedPloughEquipmentType == 'Others' &&
              _customPloughEquipmentTypeController.text.isEmpty)
            _fieldErrors['custom_eq'] = 'Please specify equipment type';
          if (_selectedPloughCapacity == null)
            _fieldErrors['asset'] = 'Please select capacity';
          else if (_selectedPloughCapacity == 'Custom / Other Capacity' &&
              _customPloughCapacityController.text.isEmpty)
            _fieldErrors['custom_cap'] = 'Please specify capacity';
        } else {
          if (_quantityController.text.isEmpty) {
        ...
        if (!isNoSlotRequired && _selectedSlots.isEmpty) {
          _fieldErrors['slots'] = 'Please select at least one time slot';
        }
      });

      // Scroll to first error
      if (_fieldErrors.containsKey('purpose') ||
          _fieldErrors.containsKey('purpose_custom') ||
          _fieldErrors.containsKey('asset') ||
          _fieldErrors.containsKey('asset_custom') ||
      ...
      UiUtils.showCenteredToast(
        context,
        AppLocalizations.of(context)!.fillAllDetails,
        isError: true,
      );
    }
  }
        ...
        child: Column(
          ...
          children: [
            ...
            _buildSectionCard(
              ...
              isError:
                  (_fieldErrors.containsKey('qty') ||
                      _fieldErrors.containsKey('purpose') ||
                      _fieldErrors.containsKey('asset')),
              child:
                  isElectrician
                      ? Column(
                      ...
                      ? Column(
                        ...
                        children: [
                          ...
                          _buildTextField(
                            ...
                            onChanged: (_) {
                              if (_fieldErrors.containsKey('qty'))
                                setState(() => _fieldErrors.remove('qty'));
                            },
                          ),
                        ],
                      )
                      ...
                      ? Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          _buildFarmerEquipmentChipsCard(widget.equipmentName ?? '', 'Sprayer'),
                          const SizedBox(height: 20),
                          _buildTextField(
                            controller: _quantityController,
                            label: 'Number of Acres',
            ...
            // Location Section - LUSH DELIVERY LOCATION
            _buildSectionCard(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      ElevatedButton.icon(
                        onPressed:
                            _isGeocodingAddress ? null : _geocodeManualAddress,
                        icon:
                            _isGeocodingAddress
                                ? const SizedBox(
                                  width: 16,
            ...
            _buildSectionCard(
              ...
              isError:
                  _fieldErrors.containsKey('date') ||
                  _fieldErrors.containsKey('slots'),
              child:
                  _isLoadingBookings
                      ? Center(
                        child: Padding(
                          ...
                          if (!isNoSlotRequired) ...[
                            ...
                            Row(
                              ...
                              children: [
                                Text(
                                  isDroneSpraying
                                      ? '${l10n.preferredTime} (Drone Arrival Time)'
                                      : isElectrician
                                          ? '${l10n.preferredTime} (Arrival Time)'
                                          : l10n.preferredTime,
                                  key: _timeSectionKey,
                                  style: const TextStyle(
                                    fontSize: 13,
              ...
              child: Column(
                ...
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: (widget.operatorIncluded ?? true)
                          ? const Color(0xFFF9FBF9)
                          : Colors.grey[100],
                      borderRadius: BorderRadius.circular(20),
                      border: (widget.operatorIncluded ?? true)
                          ? null
                    ...
                    child: SwitchListTile(
                      ...
                      subtitle: isDroneSpraying
                          ? TranslatedText(
                              (widget.operatorPrice ?? 0.0) > 0
                                  ? 'Certified operator mandatory (+ ₹${widget.operatorPrice!.toStringAsFixed(0)} operator charge included)'
                                  : 'Certified operator is mandatory & included for drone spraying',
                              style: const TextStyle(
                                fontSize: 12,
                                color: Color(0xFF2E7D32),
                                fontWeight: FontWeight.w600,
                          ...
                          : ((widget.operatorIncluded ?? true)
                              ? TranslatedText(
                                  (widget.operatorPrice ?? 0.0) > 0
                                      ? 'Operator Available (+ ₹${widget.operatorPrice!.toStringAsFixed(0)} charge)'
                                      : 'Operator is available from the owner',
                                  style: const TextStyle(
                                    fontSize: 12,
                                    color: Colors.grey,
                                    fontWeight: FontWeight.w600,
                              ...
                              : TranslatedText(
                                  'Operator not provided by owner',
                                  style: TextStyle(
                                    fontSize: 12,
                                    color: Colors.orange[800],
                                    fontWeight: FontWeight.w700,
                      ...
                      value: isDroneSpraying ? true : _includeOperator,
                      activeColor: const Color(0xFF00AA55),
                      onChanged: isDroneSpraying
                          ? (val) {
                              UiUtils.showCenteredToast(
                                context,
                                'Certified operator is mandatory for Drone Spraying services.',
                                isError: false,
                              );
                            }
                          : ((widget.operatorIncluded ?? true)
                              ...
                              : (val) {
                                  UiUtils.showCenteredToast(
                                    context,
                                    'Operator is not available from owner for this listing.',
                                    isError: true,
                                  );
                                }),
                    ),
                  ),
                  if (!(widget.operatorIncluded ?? true))
                    Container(
                        ...
                        children: [
                          ...
                          Expanded(
                            child: TranslatedText(
                              'Operator not available for this service listing.',
                              style: TextStyle(
                                fontSize: 12,
                                fontWeight: FontWeight.w600,
      ...
      children: [
        ...
        DropdownButtonFormField<String>(
          ...
          onChanged: (val) {
            ...
            if (errorKey == 'purpose' &&
                _fieldErrors.containsKey('purpose_custom'))
              setState(() => _fieldErrors.remove('purpose_custom'));
            if (errorKey == 'asset' && _fieldErrors.containsKey('asset_custom'))
              setState(() => _fieldErrors.remove('asset_custom'));
          },
        ),
  ...
  Widget _buildFarmerEquipmentChipsCard(String rawEquipments, String categoryLabel) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: Icon(
                  categoryLabel.contains('Sprayer') ? Icons.water_drop_rounded : Icons.agriculture_rounded,
              ...
              const SizedBox(width: 10),
              Text(
                '$categoryLabel Equipment & Capacity',
          ...
          const SizedBox(height: 8),
          Text(
            'Select equipment type & capacity for your requirement:',
